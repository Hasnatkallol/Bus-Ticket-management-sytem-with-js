...
// For Local Storage
const allSleeper = JSON.parse(localStorage.getItem('allSleeper')) || [];
allSleeper.forEach(id=>{
...
confirmTicket.addEventListener('click',()=>{
  ...
  const allBookedTicket = [...allSleeper,...seats]
  localStorage.setItem('allSleeper',JSON.stringify(allBookedTicket));
  seats.clear();
})

// For Reset All seat
reset.addEventListener('click',()=>{
    localStorage.removeItem('allSleeper')
...
// FOr update booking details list
 function updateBooklist() {
    ...
    allseat.forEach(id => {
      ...
      listItem.textContent = `Confirmed: ${seatText} - 1800tk`; 
