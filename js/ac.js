...
// For Local Storage
const allAc = JSON.parse(localStorage.getItem('allAc')) || [];
allAc.forEach(id=>{
...
confirmTicket.addEventListener('click',()=>{
  ...
  const allBookedTicket = [...allAc,...seats]
  localStorage.setItem('allAc',JSON.stringify(allBookedTicket));
  seats.clear();
})

// For Reset All seat
reset.addEventListener('click',()=>{
    localStorage.removeItem('allAc')
...
// FOr update booking details list
 function updateBooklist() {
    ...
    allseat.forEach(id => {
      ...
      listItem.textContent = `Confirmed: ${seatText} - 1400tk`; 
